...
During initial release validation it also runs for pushes to the exact
`feature/notification-release` branch. Branch runs build, sign, notarize, and
upload workflow artifacts without publishing a GitHub Release. Remove this
temporary branch trigger after the signing rehearsal succeeds. The workflow
also supports manual artifact-only rehearsals after it exists on the default
branch. Tag runs:
...
Create a protected GitHub Environment named `agent-release`. During the
initial rehearsal, allow the exact `feature/notification-release` branch in
addition to `agent-v*` tags, then remove the branch rule after the rehearsal.
Require a separate maintainer review and prevent self-approval when the
repository has more than one maintainer; a sole maintainer must not enable a
self-review restriction that would make releases impossible. Configure these
as environment secrets—not repository-level secrets—without placing their
values in the repository, logs, or workflow inputs:
